...
import "./utils/LiquidityLib.sol";
import "./utils/Pausable.sol";
import "./Treasury.sol";

contract LiquidityPool is Pausable, Clone, ReentrancyGuard {
    /// -----------------------------------------------------------------------
    /// Dependencies
    /// -----------------------------------------------------------------------

    using LiquidityLib for Pool;
    ...
    /// @param tradeTime The time of  the swap.
    event Buy(address indexed caller, uint256 assets, uint256 shares, uint256 swapFee, uint256 tradeTime);
    ...
    /// @param swapFee The amount of fee charged in the swap.
    /// @param tradeTime The time of  the swap.
    event Sell(address indexed caller, uint256 shares, uint256 assets, uint256 swapFee, uint256 tradeTime);
        ...
        virtual
    {
        ...
        emit Buy(msg.sender, assetsIn, sharesOut, swapFees, block.timestamp);
    }
        ...
        virtual
    {
        ...
        emit Sell(msg.sender, sharesIn, assetsOut, swapFees, block.timestamp);
    }
    ...
    function getPrice() external view virtual returns (uint256 price) {
        ...
        uint256 sharesOut = balanceOf.mulWad(1e12);
        return args().getPrice(sharesOut);
    }

    function getPrice(uint256 sharesOut) external view virtual returns (uint256 price) {
        return args().getPrice(sharesOut);
    }
